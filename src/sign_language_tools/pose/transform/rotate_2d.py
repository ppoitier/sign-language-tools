...
class RotateToReference(Transform):
    """
    ...
    def __call__(self, pose_seq: np.ndarray) -> np.ndarray:
        ...
        Args:
            pose_seq (np.ndarray): A numpy array of shape (T, L, 2).
        ...
        ref_vec = pose_seq[:, p2_idx, :] - pose_seq[:, p1_idx, :]
        print(ref_vec.shape)
        ...
        # Shape: (T, 2, 2)
        rot_mats = np.zeros((pose_seq.shape[0], 2, 2))
        rot_mats[:, 0, 0] = cos_a
        rot_mats[:, 0, 1] = sin_a
        rot_mats[:, 1, 0] = -sin_a
        rot_mats[:, 1, 1] = cos_a
        ...
        pivots = pose_seq[:, p1_idx, :][:, None, :]  # Shape: (T, 1, 2)

        # Translate to origin, rotate, then translate back
        translated_pose = pose_seq - pivots
